...
return http.build();
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {    
	    http.csrf().disable()
	        .authorizeRequests()
	        .antMatchers("/registration/**").permitAll()
	        .antMatchers("/login").permitAll().successHandler(successHandler) // Allow access to /login without form login configuration
	        .and()
	        .httpBasic() // Use basic authentication instead of form login
	        .and()
	        .logout(
	                logout -> logout
	                        .logoutRequestMatcher(new AntPathRequestMatcher("/logout"))
